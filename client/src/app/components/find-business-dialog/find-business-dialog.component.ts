...
export class FindBusinessDialogComponent implements OnInit {
  ...
  form: FormGroup;
  animationState = true;
  businessWasFetched = false;
  BusinessName: string
  businessArray: BusinessDTO[];
  daysOfWeek: string[] = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']; constructor(private geoCodeService: GeocodingApiService, private dialogRef:  MatDialogRef<FindBusinessDialogComponent>, private spinner: NgxSpinnerService, private fb: FormBuilder,
              private businessService: BusinessService,  @Inject(MAT_DIALOG_DATA)public data: any) {
    this.businessArray = [];
  }
